...
import { escapeAttribute, escapeHtml, formatDate, formatQuantity, getPlanDaySummaries, renderUnitOptions } from '../renderUtils.js';
...
export const planViewMethods = {
  ...
  renderShoppingList(dashboard) {
    ...
    const statusText = hasShoppingItems
    ...
    return `
      <details class="status-panel shopping-dropdown ${hasShoppingItems ? '' : 'ok'}">
        <summary class="shopping-summary">
          <span>
            <strong>Lista de la compra</strong>
            <small>${statusText} · ${affectedText}</small>
          </span>
          <span class="shopping-badge ${hasShoppingItems ? 'is-warning' : 'is-ok'}">
            ${hasShoppingItems ? 'Comprar' : 'Suficiente'}
          </span>
        </summary>
        <div class="shopping-dropdown-content">
          ${hasShoppingItems ? this.renderShoppingItems(dashboard) : '<p class="shopping-ok-text">Tienes alimentos suficientes para el plan actual.</p>'}
          ${this.renderShoppingExtraForm()}
          ${this.renderShoppingPurchaseActions({ totalShoppingItems, checkedShoppingItems })}
